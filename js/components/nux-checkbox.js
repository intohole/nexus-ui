        ...
        template: `
            <label class="nux-check" :class="{ 'nux-check--disabled': disabled }" @click.prevent="toggle">
                <span class="nux-check-box" :class="{ 'nux-check-box--on': modelValue }" aria-hidden="true">
                    <svg v-if="modelValue" viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="var(--nx-text-on-accent,#fff)" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 8.5 6.5 12 13 4.5"/></svg>
                </span>
